...
echo "Welcome to the NetWorkDetection setup."
echo "First we have to get arp-scan, the core where this tool is build on top."
sudo apt-get update
...
if [ "$( sudo service domoticz status | grep -i 'Home Automation System' )" != "" ] ; then 
	...
	echo "Let's check if there is already a NWD configured" 
	
	HardwareIDX=$( curl -m 10 -s "http://${DomoIP}:${DomoPort}/json.htm?type=hardware | grep -A 8 NWDhardware | grep idx | cut -d'"' -f4)
	if [ "${HardwareIDX}" == "" ] then 
		echo "No previous install detected, creating a Hardware Type" 
	...
	NewDevicesFoundIDX=$( curl -m 10 -G "http://${DomoIP}:${DomoPort}/json.htm
